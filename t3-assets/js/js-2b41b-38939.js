...
ISSUU.components.Shelf = function(config) {
  ...
  themes = {
    theme1: {
      backgroundImage: {
        small: 'http://static.' + issuuShelfConfig.domain + '/widgets/shelf/rev0/images/themes/theme1-background-small.png',
        medium: 'http://static.' + issuuShelfConfig.domain + '/widgets/shelf/rev0/images/themes/theme1-background-medium.png'
      },
    ...
    theme2: {
      backgroundImage: {
        small: 'http://static.' + issuuShelfConfig.domain + '/widgets/shelf/rev0/images/themes/theme2-background-small.png',
        medium: 'http://static.' + issuuShelfConfig.domain + '/widgets/shelf/rev0/images/themes/theme2-background-medium.png'
      },
      titleTextColor: '#FFFFFF',
    ...
    theme3: {
      backgroundImage: {
        small: 'http://static.' + issuuShelfConfig.domain + '/widgets/shelf/rev0/images/themes/theme3-background-small.png',
        medium: 'http://static.' + issuuShelfConfig.domain + '/widgets/shelf/rev0/images/themes/theme3-background-medium.png'
      },
  ...
  that.cfg.shadowImage = typeof config.shadowImage !== 'undefined' ? config.shadowImage : 'http://static.' + issuuShelfConfig.domain + '/widgets/shelf/rev0/images/shadow.png';
  ...
  that.cfg.target = typeof config.target !== 'undefined' ? config.target : '_top';
  ...
  that.cfg.backgroundImage = typeof config.backgroundImage !== 'undefined' ? config.backgroundImage : (that.cfg.thumbSize === 'small' ? 'http://static.' + issuuShelfConfig.domain + '/widgets/shelf/rev0/images/background_small.png' : 'http://static.' + issuuShelfConfig.domain + '/widgets/shelf/rev0/images/background_medium.png');
  ...
  if (sharedData.pagingType === 'local') {
    ...
    that.normalize = function (callback) {
      ...
      for (i = 0; i < feedData.items.length; i += 1) {
        ...
        //itemData.link = feedData.items[i].link || fields.link;
        itemData.documentId = feedData.items[i].documentId || fields.documentId;
        itemData.link = '?option=com_ebooks&view=info&doc_id=' + itemData.documentId || fields.link;
    ...
    that.renderPage = function () {
      ...
      jQ132('.issuu-doc-outer').hover(function (evt) {
        var docId = jQ132(this).find('.issuu-cover-link').attr('id').substr(jQ132(this).find('.issuu-cover-link').attr('id').lastIndexOf('_') + 1);
        jQ132(this).append('<a href="' + jQ132(this).find('.issuu-cover-link').attr('href') + '" target="' + that.cfg.target + '" class="tooltip" style="left: ' + ((jQ132(this).width() - 17) / 2) + 'px;">read now</a>');
        jQ132(this).find('.tooltip').click(function (evt) {
          if (folderId) {
            that.logEvent({
              action: 'click',
        ...
        jQ132(this).find('.tooltip').animate({
          opacity: '0.9'
        }, 150, 'swing');
      }, function (evt) {
        jQ132(this).find('.tooltip').animate({
          ...
          jQ132(this).find('.tooltip').remove();
        });
      });
    };
    ...
    that.normalize = function (callback) {
      var feedData, normalizedData, items, fields, i, itemData, pageSize;
      normalizedData = [];
      if (that.cfg.tabs) {
        feedData = sharedData.tabs[sharedData.currentTab].feedData;
      } else {
      ...
      if (feedData.items && feedData.items.length > 0) {
        ...
        pageSize = feedData.itemsPerPage;
        ...
        for (i = 0; i < items.length; i += 1) {
          ...
          itemData.link = that.getDocumentUrl(items[i].groupItem.documentUsername, items[i].groupItem.name) + (itemData.page === 1 ? '' : ('/' + itemData.page));
          normalizedData.push(itemData);
          morePages[itemData.documentId] = {
            pages: []
          };
        }
        pageSize = feedData.rsp._content.result.itemsPerPage;
      } else if (feedData.response && feedData.response.docs && feedData.response.docs.length > 0) {
        items = feedData.response.docs;
        for(i=0; i<items.length; i+=1) {
          itemData = {};
          itemData.title = items[i].title;
          itemData.user = items[i].username;
          itemData.documentId = items[i].documentId;
          itemData.page = 1;
          itemData.link = that.getDocumentUrl(items[i].username, items[i].docname);
          normalizedData.push(itemData);
          morePages[itemData.documentId] = {
            pages: []
          };
        }
        pageSize = feedData.response.docs.length;
      } else {
        jQ132('#' + that.cfg.containerId + ' .issuu-loading').hide();
        that.renderEmptyMsg();
        return;
      ...
      if (that.cfg.tabs) {
        ...
        sharedData.normalizedData = normalizedData;
        sharedData.normalizedData.pageSize = pageSize;
      }
      if (callback) {
        callback();
      }
  ...
  that.renderShelf = function () {
    ...
    shelfHtml += '<div class="issuu-paging"><a href="#" class="issuu-prev" style="height: ' + that.cfg.rowHeight + 'px;"><img src="http://static.' + issuuShelfConfig.domain + '/widgets/shelf/rev0/images/arrow-prev.gif" alt="Prev"></a> <a href="#" class="issuu-next" style="height: ' + that.cfg.rowHeight + 'px;"><img src="http://static.' + issuuShelfConfig.domain + '/widgets/shelf/rev0/images/arrow-next.gif" alt="Next"></a></div>';
    //shelfHtml += '<a href="http://issuu.com/shelf" title="Create your own shelf" target="_blank" class="shelf-by-issuu">Shelf by Issuu</a>'
    shelfHtml += '</div>';
    if (that.cfg.roundedCorners && (!jQ132.browser.msie || (jQ132.browser.msie && jQ132.browser.version * 1 > 6))) {
      shelfHtml += '<img src="http://static.' + issuuShelfConfig.domain + '/widgets/shelf/rev0/images/rc/rc_tl.png" class="issuu-rctl">';
      shelfHtml += '<img src="http://static.' + issuuShelfConfig.domain + '/widgets/shelf/rev0/images/rc/rc_tr.png" class="issuu-rctr">';
      shelfHtml += '<img src="http://static.' + issuuShelfConfig.domain + '/widgets/shelf/rev0/images/rc/rc_bl.png" class="issuu-rcbl">';
      shelfHtml += '<img src="http://static.' + issuuShelfConfig.domain + '/widgets/shelf/rev0/images/rc/rc_br.png" class="issuu-rcbr">';
    }
    shelfHtml += '<span class="issuu-loading"><img src="http://static.' + issuuShelfConfig.domain + '/widgets/shelf/rev0/images/spinner.gif" alt="Loading"></span>';
    shelfHtml += '</div>';
  ...
  that.loadDefault = function () {
    ...
    newConfig.feedUrl = 'http://search.issuu.com/ds?q=&startIndex=0&pageSize=20&sortBy=rating&responseParams=*';
  ...
  that.renderPublication = function (publicationData) {
    ...
    if (that.cfg.effect3d && (!jQ132.browser.msie || (jQ132.browser.msie && jQ132.browser.version * 1 > 6))) {
      publicationHtml += '<img src="http://static.' + issuuShelfConfig.domain + '/widgets/shelf/rev0/images/3dEffect.png" class="issuu-threedee">';
    }
    publicationHtml += '</a>';
    publicationHtml += '</div>';
    publicationHtml += '</div>';
  ...
  that.getDocumentThumbUrl = function (documentId, page) {
    return 'http://image.' + issuuShelfConfig.domain + '/' + documentId + '/jpg/page_' + page + '_thumb_' + that.cfg.thumbSize + '.jpg';
	};
	
	that.getUserUrl = function (user) {
		return 'http://' + issuuShelfConfig.domain + '/' + user;
	};
	
	that.getDocumentUrl = function (user, docName) {
		return 'http://' + issuuShelfConfig.domain + '/' + user + '/docs/' + docName;
	};
  ...
  that.logEvent = function (params) {
    var dataUrl = 'http://sidebar.issuu.com/s';
    params.folderId = folderId;
    params.shelfVersion = shelfVersion;
    params.jsoncallback = '?'
    try {
      jQ132.ajax({
        type: 'GET',
